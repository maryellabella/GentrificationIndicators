...
clipped_gdf.drop(columns=['nbhd', 'township_c', 'township_n', 'triad_code', 'triad_name'])
print(clipped_gdf[['pri_neigh', 'sec_neigh']].head())
```
...
```{python}
ps_merged_gdf.drop(columns=['nbhd', 'shape_area', 'triad_code', 'township_n', 'township_c', 'sec_neigh'], inplace=True)
```

```{python}
ps_merged_gdf_2023 = ps_merged_df[ps_merged_df['year'] == 2023]

fig, ax = plt.subplots(figsize=(8, 5), dpi=75)

ps_merged_gdf_2023.plot(column='parcels_sold', ax=ax, legend=True,
                        cmap='Blues', edgecolor='lightgray', linewidth=0.5)

ax.set_title('Number of Parcels Sold by Neighborhood in 2023')
ax.set_xticks([])
ax.set_yticks([])
