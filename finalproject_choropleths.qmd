...
print(merged_gdf_shapefile.columns)
```
...
av_df['certified_tot'] = pd.to_numeric(av_df['certified_tot'], errors='coerce')
...
ax.set_title('Average Assessed Value by Geometry (Neighborhood Area, Year 2023)')
ax.set_xticks([])
ax.set_yticks([])
...
#top neighborhoods with changes 
...
```{python}
#chicago proper boundary
zip_neighborhood_folder = 'zip_neighborhood'
zip_neighborhood_path = os.path.join(path, zip_neighborhood_folder, 'geo_export_3c8ecaa3-ca28-4d2e-b1a8-d6d1d1701126.shp')
zip_neighborhood_gdf = gpd.read_file(zip_neighborhood_path)
print(zip_neighborhood_gdf.columns)
print(zip_neighborhood_gdf.head())
```
